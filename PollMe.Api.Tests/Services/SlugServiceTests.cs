...
public class SlugServiceTests
{
    ...
    [Fact]
    public async Task Generate_ExhaustedAttempts_ThrowsInvalidOperationException()
    {
        var pollRepo = Substitute.For<IPollRepository>();
        var sut = new SlugService(pollRepo);
        pollRepo.SlugExistsAsync(Arg.Any<string>()).Returns(true);

        await Assert.ThrowsAsync<InvalidOperationException>(() => sut.GenerateUniqueSlugAsync());
    }
}
