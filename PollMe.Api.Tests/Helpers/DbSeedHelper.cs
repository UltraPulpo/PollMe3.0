...
public static class DbSeedHelper
{
    ...
    {
        return await conn.ExecuteScalarAsync<int>(
            "INSERT INTO polls (creator_id, slug, question, mode, visibility, created_at) VALUES (@CreatorId, @Slug, @Question, @Mode, @Visibility, @CreatedAt); SELECT last_insert_rowid();",
            new { CreatorId = creatorId, Slug = slug, Question = question, Mode = mode, Visibility = visibility, CreatedAt = DateTime.UtcNow.ToString("o") });
    }
